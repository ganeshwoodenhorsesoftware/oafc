...
Composer based Drupal 9 project.
...
### Database import
If for some reason the database import returns an error, you may use try these alternative steps :
```
lando drush sql-sync @dev @self --target-dump=/app
lando db-import oafc_[date].sql.gz
rm oafc_[date].sql.gz
```
Sometimes Drush runs into issues with .gz files.

## Lando Tooling
- Check code standards: `lando code-standards`.
  - Checks custom code for code violations.
- Theme Tooling:
  - Compile: `lando theme-compile`.
  - Watch: `lando theme-watch`.
...
#### Troubleshooting
You may need to create a new user in MySQL since database dumps can have the
wrong definer for views or stored procedures. See
https://stackoverflow.com/a/19707173. For example, locally in a DrupalVM
there's no `oafc@localhost` user but the database dump from another server uses
that. That causes fatal errors when trying to create a contact. A work-around is
to create the user in the VM.
