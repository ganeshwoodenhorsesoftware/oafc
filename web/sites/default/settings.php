 ...
 * @endcode
 */
$settings['hash_salt'] = 'mv_qW_4J19abi3kBhUECFnLHp23ki5k2Fxknhm8NHakgQYJJLp-Yhfsb1VvTmHWTcvUkpxC4iA';
 ...
 */
$settings['deployment_identifier'] = '2';
...
if (getenv('KUBERNETES_SERVICE_HOST')) {
  ...
  // In Kubernetes, the proxy IP is not fixed, so we trust the connecting IP.
  $settings['reverse_proxy'] = TRUE;
...
if (getenv('PAYPAL_CLIENT_ID') && getenv('PAYPAL_SECRET')) {
  $config['commerce_payment.commerce_payment_gateway.paypal_checkout']['configuration']['client_id'] = getenv('PAYPAL_CLIENT_ID');
  $config['commerce_payment.commerce_payment_gateway.paypal_checkout']['configuration']['secret'] = getenv('PAYPAL_SECRET');
}

if (getenv('PRODUCTION')) {
  $settings['trusted_host_patterns'] = [
    '^oafc\.oafc\.acro\.website$',
    '^www\.oafc\.on\.ca$',
    '^oafc\.on\.ca$',
    '^oafc\.prod\.gke\.acro\.website$',
  ];
  $settings['hash_salt'] = '13m9wHX7toUQlwl9fraab32b8h783ruBLr0vR_IkAQaOeJcXhcrIJcXvzMJVAhMPReBbStuiWw';
  $config['config_split.config_split.local']['status'] = FALSE;
  $config['vbulletin.settings']['vbpath'] = realpath($app_root . '/forum');
}

$app_env = getenv('ENVIRONMENT_TYPE') ?: 'local';

if ($app_env == 'development' || $app_env == 'staging') {
  $config['symfony_mailer_lite.symfony_mailer_lite_transport.sendmail']['configuration']['query']['command'] = '/usr/bin/msmtp -t -i';
  $config['search_api.server.default']['backend_config']['connector_config']['host'] = 'solr';
  $config['search_api.server.default']['backend_config']['connector_config']['core'] = 'drupal';
}

if (getenv('MAILCHIMP_API_KEY')) {
  // MailChimp.
  $config['mailchimp.settings']['api_key'] = getenv('MAILCHIMP_API_KEY');
  $config['mailchimp.settings']['domain'] = 'www.oafc.on.ca';
}
